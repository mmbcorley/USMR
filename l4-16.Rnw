...
\subsection[Criteria]{Setting Criteria: $\alpha$}
...
\item \emph{general psychology:} want a good compromise between the
  two possible errors
\pause
\item in psychology, the most used criterion is \textcolor{red!50!black}{$p(\text{H$_{\text{0}}$})
  < .05$} (usually abbreviated as \textcolor{red!50!black}{$p < .05$})
\end{itemize}
...
~
{\huge
 psychological ``results'' can have as much as a \term{1 in
    20} probability of being entirely due to chance!}
\pause
\spc
\begin{itemize}
\item a difference in DVs is said to be \term{significant} if $p < .05$
\end{itemize}

\end{frame}


\section[df]{Degrees of Freedom (df)}

\subsection{How they're used}
