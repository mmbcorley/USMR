...
#
opts_chunk$set(fig.path='img/auto-',fig.align='center',fig.show='hide',size='scriptsize',warning=FALSE,tidy=TRUE,tidy.opts=list(keep.blank.line=FALSE, width.cutoff=92),fig.width=5,fig.height=4.15,par=TRUE)
# set everything to handlable numbers of digits (reclaim screen space!)
options(replace.assign=TRUE,width=92,digits=2)
...
\begin{frame}{The R Team}
  \begin{block}{Course Leaders}
    Martin Corley, Room G8, 7GS \atright{\url{Martin.Corley@ed.ac.uk}}\\
    Pavel Iosad, Room 3.08, DSB \atright{\url{Pavel.Iosad@ed.ac.uk}}
  \end{block}
  \begin{alertblock}{Teaching Fellow}
  Milan Val\'{a}\v{s}ek, Room G6, 7GS \atright{\url{Milan.Valasek@ed.ac.uk}}\\
  \textcolor{gray}{Luna de Ferrari, Room G6, 7GS \atright{\url{luna.deferrari@ed.ac.uk}}}
    \end{alertblock}
%  \spc
...
\end{tabular}  
\end{block}
\atright{\pgfimage[width=.25\linewidth]{img/a-team-van.jpg}}
\end{frame}
...
\begin{frame}
  ...
  \item optional homeworks and solutions
  \end{itemize}
...
wordcloud(pp, scale=c(5,0.5), max.words=150, random.order=FALSE, rot.per=0.35, colors=brewer.pal(12,'Dark2'))
@ 
% should work now  
\end{frame}

\begin{frame}{The 150 Most Frequent Words in \emph{Pride and Prejudice}}
\cimage[.5833\linewidth]{img/auto-PP-1}  
\end{frame}
...
\cimage[.6667\linewidth]{img/articles.png}
\spc
...
\begin{frame}{R Usage: Jobs on indeed.com}
 ...
 \cimage[.6667\linewidth]{img/jobs_indeed}
 \spc
 \atright{\small\url{http://r4stats.com/articles/popularity/}}
...
\begin{frame}
  \frametitle{R Usage: Analytic Professionals}

\cimage[.6667\linewidth]{img/usage_rexer}
\spc
\atright{\small\url{http://bit.ly/1mYNSy7}}
...
\item Idea \ra{} Design
  \begin{itemize}
  \item roughly, ``how to have research ideas''\footnote{Much more on
      this in Psychological Research Skills}
  \end{itemize}
...
\begin{frame}{Results}
\cimage[.9\linewidth]{img/sanford}
\end{frame}
...
\begin{itemize}
  ...
  \end{itemize}
\cimage[.6\linewidth]{img/auto-heads3-1}
